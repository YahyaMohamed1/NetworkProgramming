...
def checkans():                 # Check if the answer is correct or not and return True/False
    var = e1.get().strip()  # Remove any leading/trailing whitespaces

    client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client_socket.connect(('localhost', 8000))

    word = client_socket.recv(1024).decode()    # Receive the word from the server
    label.config(text=word)                     # Display the word on the screen

    client_socket.sendall(var.encode())         # Send the answer to the server
    result = client_socket.recv(1024).decode()  # Receive the result from the server
    result = bool(result)
